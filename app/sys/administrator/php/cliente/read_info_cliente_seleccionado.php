...
session_start();
  require_once '../../../conexion.php';
    ...
    DATE_FORMAT(pg.fecha_desde,'%d-%m-%Y') AS fecha_desde,
    DATE_FORMAT(pg.fecha_hasta,'%d-%m-%Y') AS fecha_hasta, 
    pg.estado AS estado_pago, g.nombre AS giro
    FROM cliente c
    JOIN pago_cliente pg 
    ON c.id = pg.id_cl
    JOIN planes pl 
    ON c.plan_comprado = pl.id
    JOIN giros g 
    ON c.giro = g.id
    WHERE c.id = $id
    GROUP BY c.id";

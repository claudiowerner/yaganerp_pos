...
function descargarCalendario(año, mes)
{
    ...
    return $.ajax({
        url: "graficos/php/calendario/calendario/calendario.php",
        data: datos,
        type: "POST",
        async: false
    }).responseText;
...
function semana(template, dia_semana, dia_mes,  mes, año)
{
    ...
    while(dia_semana<8)
    {
        ...
        template += 
        `<td >
            <button id='${año}-${mes}-${(fecha)}' class='btn btn-primary' onClick='crearGraficoVentaHora("${año}-${mes}-${(fecha)}")' disabled>
                <h3>${(fecha)}</h3>
            </button>
        </td>`;
        if(dia_semana==7)
